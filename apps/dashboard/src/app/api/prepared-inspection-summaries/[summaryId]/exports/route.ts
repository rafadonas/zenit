...
const EXPORT_SCHEMA_VERSION = "prepared-inspection-summary-csv-v1";
...
function hasSafeExportHeaders(response: Response): boolean {
  return response.headers.get("content-type")?.split(";", 1)[0]?.trim() === "text/csv" &&
    ...
    response.headers.get("x-zenit-data-status") === "prepared" &&
    response.headers.get("x-zenit-location-status") === "simulated" &&
...
export async function POST(request: NextRequest, context: RouteContext): Promise<NextResponse> {
  ...
  if (!requestOriginMatches(request.headers.get("origin"), config.publicOrigin)) {
    return NextResponse.json({ detail: "Cross-origin export rejected" }, { status: 403 });
  }
  ...
  try {
    upstream = await fetch(`${baseUrl}/v1/prepared-inspection-summaries/${summaryId}/exports`, {
  ...
  return new NextResponse(content, {
    ...
    headers: {
      ...
      "Content-Disposition": `attachment; filename="zenit-prepared-summary-${summaryId}.csv"`,

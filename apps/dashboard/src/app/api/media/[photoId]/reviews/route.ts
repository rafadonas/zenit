...
import {
  ...
  parsePhotoReviewSubmission, requestOriginMatches, SESSION_COOKIE_NAME,
...
function redirect(origin: string, status: string): NextResponse {
  ...
  destination.searchParams.set("review", status);
...
export async function POST(request: NextRequest, context: RouteContext): Promise<NextResponse> {
  ...
  if (!requestOriginMatches(request.headers.get("origin"), config.publicOrigin)) {
    return NextResponse.json({ detail: "Cross-origin review rejected" }, { status: 403 });
  }
  ...
  if (!submission || !csrfTokensMatch(submission.csrfToken, request.cookies.get(CSRF_COOKIE_NAME)?.value)) {
  ...
  const body = {
    decision: submission.decision, quality_status: submission.qualityStatus,
    ruler_status: submission.rulerStatus,
    ...(submission.rationale ? { rationale: submission.rationale } : {}),
    ...(submission.supersedesReviewId ? { supersedes_review_id: submission.supersedesReviewId } : {}),
  };
  const baseUrl = process.env.INTERNAL_API_URL ?? "http://localhost:8000";
  let upstream: Response;
  try {
    upstream = await fetch(`${baseUrl}/v1/media/${photoId}/reviews`, {
      method: "POST", cache: "no-store", body: JSON.stringify(body),
      headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json", "Idempotency-Key": submission.idempotencyKey },
    });
  ...
  const statuses: Record<number, string> = { 403: "forbidden", 404: "missing", 409: "conflict", 422: "invalid" };

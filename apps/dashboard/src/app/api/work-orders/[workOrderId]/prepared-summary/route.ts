...
import {
  ...
  parsePreparedSummarySubmission, requestOriginMatches, SESSION_COOKIE_NAME,
} from "../../../../../lib/session-security";

interface RouteContext { params: Promise<{ workOrderId: string }>; }

function redirect(origin: string, status: string): NextResponse {
  const destination = new URL("/photo-reviews", origin);
  destination.searchParams.set("summary", status);
...
export async function POST(request: NextRequest, context: RouteContext): Promise<NextResponse> {
  ...
  if (!requestOriginMatches(request.headers.get("origin"), config.publicOrigin)) {
    return NextResponse.json({ detail: "Cross-origin summary request rejected" }, { status: 403 });
  }
  const { workOrderId } = await context.params;
  if (!isUuid(workOrderId)) return redirect(config.publicOrigin, "invalid");
  let form: FormData;
  try { form = await request.formData(); } catch { return redirect(config.publicOrigin, "invalid"); }
  const submission = parsePreparedSummarySubmission(form);
  ...
  try {
    upstream = await fetch(`${baseUrl}/v1/work-orders/${workOrderId}/prepared-summary`, {
      method: "POST", cache: "no-store",
      body: JSON.stringify({ generation_rationale: submission.generationRationale }),
      headers: {
  ...
  if (upstream.ok) return redirect(config.publicOrigin, "generated");

...
import {
  ...
  parsePreparedMowingResourcePlanSubmission, requestOriginMatches, SESSION_COOKIE_NAME,
...
function redirect(origin: string, status: string): NextResponse {
  ...
  destination.searchParams.set("resource_plan", status);
...
export async function POST(request: NextRequest, context: RouteContext): Promise<NextResponse> {
  ...
  if (!requestOriginMatches(request.headers.get("origin"), config.publicOrigin)) {
    return NextResponse.json({ detail: "Cross-origin resource-plan request rejected" }, { status: 403 });
  }
  ...
  const submission = parsePreparedMowingResourcePlanSubmission(form);
  ...
  const body = {
    team_reference: submission.teamReference,
    equipment_reference: submission.equipmentReference,
    planning_rationale: submission.planningRationale,
    ...(submission.supersedesPlanId ? { supersedes_plan_id: submission.supersedesPlanId } : {}),
  };
  const baseUrl = process.env.INTERNAL_API_URL ?? "http://localhost:8000";
  let upstream: Response;
  try {
    upstream = await fetch(
      `${baseUrl}/v1/prepared-mowing-orders/${mowingOrderId}/resource-plans`,
      {

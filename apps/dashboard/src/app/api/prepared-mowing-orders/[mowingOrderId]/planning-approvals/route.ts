...
import {
  ...
  parsePreparedMowingPlanningApprovalSubmission, requestOriginMatches, SESSION_COOKIE_NAME,
...
function redirect(origin: string, status: string): NextResponse {
  ...
  destination.searchParams.set("planning_approval", status);
...
export async function POST(request: NextRequest, context: RouteContext): Promise<NextResponse> {
  ...
  if (!requestOriginMatches(request.headers.get("origin"), config.publicOrigin)) {
    return NextResponse.json({ detail: "Cross-origin planning approval rejected" }, { status: 403 });
  }
  ...
  const submission = parsePreparedMowingPlanningApprovalSubmission(form);
  ...
  const body = {
    readiness_assessment_id: submission.readinessAssessmentId,
    decision: submission.decision,
    decision_rationale: submission.decisionRationale,
    ...(submission.supersedesApprovalId ? {
      supersedes_approval_id: submission.supersedesApprovalId,
    } : {}),
  };
  ...
  try {
    upstream = await fetch(
      `${baseUrl}/v1/prepared-mowing-orders/${mowingOrderId}/planning-approvals`,
      {

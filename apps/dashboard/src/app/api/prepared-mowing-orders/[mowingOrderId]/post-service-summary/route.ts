...
function redirect(origin: string, status: string): NextResponse {
  ...
  destination.searchParams.set("mowing_summary", status);
...
export async function POST(request: NextRequest, context: RouteContext): Promise<NextResponse> {
  ...
  if (!requestOriginMatches(request.headers.get("origin"), config.publicOrigin)) {
    return NextResponse.json(
      { detail: "Cross-origin mowing summary request rejected" },
      { status: 403 },
    );
  }
  ...
  try {
    upstream = await fetch(`${baseUrl}/v1/prepared-mowing-orders/${mowingOrderId}/post-service-summary`, {

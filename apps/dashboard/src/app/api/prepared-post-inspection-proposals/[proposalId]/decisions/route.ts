...
import {
  ...
  parsePreparedProposalReviewSubmission, requestOriginMatches, SESSION_COOKIE_NAME,
...
function redirect(origin: string, status: string): NextResponse {
  ...
  destination.searchParams.set("proposal_review", status);
...
export async function POST(request: NextRequest, context: RouteContext): Promise<NextResponse> {
  ...
  if (!requestOriginMatches(request.headers.get("origin"), config.publicOrigin)) {
    return NextResponse.json({ detail: "Cross-origin proposal review rejected" }, { status: 403 });
  }
  const { proposalId } = await context.params;
  if (!isUuid(proposalId)) return redirect(config.publicOrigin, "invalid");
  let form: FormData;
  try { form = await request.formData(); } catch { return redirect(config.publicOrigin, "invalid"); }
  const submission = parsePreparedProposalReviewSubmission(form);
  ...
  try {
    upstream = await fetch(
      `${baseUrl}/v1/prepared-post-inspection-proposals/${proposalId}/decisions`,
      {

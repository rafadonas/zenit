...
interface PageProps { searchParams: Promise<{ export?: string }>; }
...
async function loadExceptions(): Promise<MowingPostServiceExceptionCollection | null> {
  ...
  if (response.status === 401) return null;
  if (!response.ok) throw new Error(`Mowing post-service exceptions returned HTTP ${response.status}`);
  const payload: unknown = await response.json();
...
function exportMessage(status?: string): string | null {
  if (status === "missing") return "O resumo pós-serviço simulado não foi encontrado ou não está acessível.";
  if (status === "conflict") return "A chave da exportação entrou em conflito com outra solicitação.";
  if (status === "invalid") return "Informe um propósito válido para exportar o resumo.";
  if (status === "unsafe-response") return "A exportação foi bloqueada porque o CSV não confirmou os rótulos simulados.";
  if (status === "service-unavailable") return "O serviço de exportação não está disponível agora.";
  return null;
}
...
export default async function MowingPostServiceSummariesPage({ searchParams }: PageProps) {
  const [session, summaries, exceptions, query] = await Promise.all([
    loadDashboardSession(), loadSummaries(), loadExceptions(), searchParams,
  ]);
  const operationMessage = exportMessage(query.export);
  ...
  return <main className="recommendations-shell">
    ...
    {!session || !summaries ? <section className="reviewer-session"><div><strong>Sessão necessária</strong><span>Entre para acessar resumos pós-serviço da sua rodovia.</span></div><Link className="primary-button" href="/login">Entrar</Link></section> : <><section className="reviewer-session"><div><strong>{session.user.display_name}</strong><span>{session.user.email}</span></div><div className="role-list">{session.road_roles.map((role) => <span key={`${role.road_code}-${role.role}`}>{role.road_code} · {role.role}</span>)}</div></section><section className="queue-summary"><strong>{summaries.result_count}</strong><span>resumo(s) pós-serviço acessível(is)</span>{summaries.truncated ? <small>Lista limitada a {summaries.limit} itens.</small> : null}</section><p className="quality-note"><span>i</span><span><strong>Escopo seguro</strong> {summaries.warning}</span></p><section className="prepared-summary-grid" aria-label="Resumos pós-serviço simulados">{summaries.items.length === 0 ? <div className="queue-empty"><h2>Nenhum resumo</h2><p>Não há resumo pós-serviço simulado disponível para seu papel.</p></div> : summaries.items.map((item) => { const exception = exceptionBySummary.get(item.summary_id); return <article className="prepared-summary-card" key={item.summary_id}><div className="prepared-summary-heading"><div><p className="eyebrow">Ordem {item.mowing_order_id}</p><h2>Resumo pós-serviço</h2></div><span className="status-pill simulated">Simulado</span></div><dl className="summary-metrics"><div><dt>Mínima</dt><dd>{cm(item.minimum_height_cm)}</dd></div><div><dt>Média</dt><dd>{cm(item.mean_height_cm)}</dd></div><div><dt>Máxima</dt><dd>{cm(item.maximum_height_cm)}</dd></div></dl><div className="summary-classes"><span>N1 {item.n1_count}</span><span>N2 {item.n2_count}</span><span>N3 {item.n3_count}</span></div><p className="summary-rationale">{item.generation_rationale}</p><small>{item.summary_policy_version} · gerado {formatDate(item.generated_at)} · {item.measurement_count} medições · {item.accepted_photo_review_count} fotos aceitas</small>{exception ? <div className="post-inspection-proposal"><div><strong>{exception.recommendation === "inspect_follow_up" ? "Inspeção de seguimento indicada" : "Monitoramento indicado"}</strong><span>Máxima {cm(exception.maximum_height_cm)} · limiar {cm(exception.applicable_threshold_cm)}</span></div><span className="status-pill review">Revisão humana obrigatória</span><small>Exceção simulada; não conclui roçada, não atualiza mapa e não autoriza campo.</small></div> : null}<form action={`/api/prepared-mowing-post-service-summaries/${item.summary_id}/exports`} className="summary-export-form" method="post"><input name="csrf_token" type="hidden" value={session.csrfToken} /><input name="idempotency_key" type="hidden" value={randomUUID()} /><label htmlFor={`mowing-export-purpose-${item.summary_id}`}>Propósito da exportação</label><input defaultValue="Compartilhar pós-serviço simulado para revisão" id={`mowing-export-purpose-${item.summary_id}`} maxLength={2000} name="export_purpose" required /><button className="secondary-button" type="submit">Baixar CSV simulado</button><small>O download gera auditoria e mantém bloqueio de relatório oficial, treino e autorização de campo.</small></form></article>; })}</section></>}
  </main>;
}

...
import { POST } from "../app/api/prepared-mowing-orders/[mowingOrderId]/planning-approvals/route";
import { CSRF_COOKIE_NAME, SESSION_COOKIE_NAME } from "./session-security";

const csrf = "a".repeat(64);
const orderId = "73000000-0000-4000-8000-000000000002";
const context = { params: Promise.resolve({ mowingOrderId: orderId }) };

afterEach(() => vi.unstubAllGlobals());

describe("prepared mowing planning-approval proxy", () => {
  it("forwards planning-only fields", async () => {
    ...
    const body = new URLSearchParams({
      ...
      readiness_assessment_id: "73000000-0000-4000-8000-000000000003",
      decision: "approved_for_planning", decision_rationale: "Aprovar cenário preparado",
      operational_approval_satisfied: "true", authorizes_field_work: "true",
    });
    const request = new NextRequest("http://localhost:3000/api/planning-approval", {
      method: "POST", body,
      headers: {

...
import { POST } from "../app/api/prepared-post-inspection-proposals/[proposalId]/decisions/route";
import { CSRF_COOKIE_NAME, SESSION_COOKIE_NAME } from "./session-security";

const proposalId = "60000000-0000-4000-8000-000000000002";
const csrf = "a".repeat(64);
const context = { params: Promise.resolve({ proposalId }) };

afterEach(() => vi.unstubAllGlobals());

describe("prepared proposal human-review proxy", () => {
  it("forwards only an allowlisted CSRF-checked decision", async () => {
    ...
    const body = new URLSearchParams({
      ...
      rationale: "Manter monitoramento preparado", authorizes_field_work: "true",
    });
    const request = new NextRequest("http://localhost:3000/api/proposal-review", {
      method: "POST", body,
      headers: {
    ...
    expect(JSON.parse(String(options.body))).toEqual({
      decision: "adjusted", adjusted_recommendation: "monitor",
      rationale: "Manter monitoramento preparado",
    });
  });

...
import { POST } from "../app/api/prepared-mowing-post-service-summaries/[summaryId]/exports/route";
...
function request(): NextRequest {
  ...
  return new NextRequest(
    `http://localhost:3000/api/prepared-mowing-post-service-summaries/${summaryId}/exports`,
    {
      method: "POST", body,
      headers: {
...
function safeUpstream(content = "export_notice,summary_id\r\nSIMULATED,summary\r\n"): Response {
  ...
  return new Response(content, {
    ...
    headers: {
      ...
      "X-Zenit-Export-Schema-Version": "simulated-mowing-post-service-summary-csv-v1",
...
describe("simulated mowing post-service CSV dashboard proxy", () => {
  ...
  it("forwards only the purpose and returns a verified simulated CSV", async () => {
    ...
    expect(response.headers.get("x-zenit-location-status")).toBe("not_collected");
